...
// ctx.fillTextFormatted(`hi, hello, world!`, x, y += 40);
    ...
    function wsClose(e) {
     ...
     document.getElementById("snackbar").innerHTML = "Disconnected... Reason:" + e.reason;
      window.showsnack();
        wsCleanup();
        gameReset();
        setTimeout(function() {
    ...
    function sendChat(text) {
        ...
        wsSend(writer);
      window.gtag('event', 'agario_chat_send');
    }
    ...
    function drawChat() {
        ...
        var latestMessages = chat.messages.slice(-25);
        var lines = [];
    ...
    function drawStats() {
        ...
        ctx.font = "14px Ubuntu";
        ctx.fillStyle = settings.darkTheme ? "#66e" : "#33F";
    ...
    function drawGame() {
        ...
        if (!isNaN(stats.score)) {
            mainCtx.font = "24px Ubuntu";
            mainCtx.fillText("Score: " + stats.score, 2, height);
            height += 24;
        }
        mainCtx.font = "16px Ubuntu";
        ...
        if (minionControlled) {
            ...
            mainCtx.fillStyle = "#a5a2ee";
            var text = "You are in minion mode, press Q to switch back.";
...
if(location.hash) {
window.setserver(location.hash.slice(1));
  ...
  location.protocol = "https:";
    ...
    window.setserver = function(arg) {
        if (wsUrl === arg) return;
        wsInit(arg);
      window.gtag('event', arg);
    };
    window.spectate = function(a) {
        wsSend(UINT8_CACHE[1]);
        stats.maxScore = 0;
        hideESCOverlay();
      window.gtag('event', 'agario_spectate');
    };
